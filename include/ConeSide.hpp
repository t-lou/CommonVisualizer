...
namespace loco
{
  ...
  public:
    ConeSide(const std::vector<float> &positions, const std::vector<float> &radius,
             const std::vector<float> &ends, const std::vector<bool> &is_convex,
        ...
        CylinderSide(positions, radius, colors, id_program)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, is_convex.size(), &begin, GL_STATIC_DRAW); // std::vector<bool>.front()...
    }

    ConeSide(const std::vector<float> &positions, const std::vector<float> &radius,
             const std::vector<bool> &is_convex,
             const std::vector<Vec> &colors, const GLuint id_program) :
        ConeSide(positions, radius, std::vector<float>(radius.size(), 0.0f), is_convex, colors, id_program)
    {}

    ConeSide(const std::vector<float> &positions, const std::vector<float> &radius,
             const std::vector<float> &ends,
             const std::vector<Vec> &colors, const GLuint id_program) :
        ConeSide(positions, radius, ends, std::vector<bool>(radius.size(), true), colors, id_program)
    {}
