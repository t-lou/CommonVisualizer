...
int main() {
  ...
  if (visualizer.isInited()) {
    ...
    float vertices[] = {};
    int indices[] = {};
    //    visualizer.addMesh(std::vector<float>(mesh, mesh + 9),
    //    loco::color::WHITE); visualizer.addMesh(std::vector<float>(mesh + 9,
    //    mesh + 18), loco::color::WHITE);
    ...
    //    visualizer.addPointCloud(std::vector<float>(mesh + 9, mesh + 18),
    //    std::vector<float>(color + 12, color + 24), 0.1f);
