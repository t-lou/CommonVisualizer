/**
 * One of motivations of CommonVisulizer is to visualize result of other
 * programs in a convenient way, for example bullet physics engine, whose
 * visualization requires self implemeted drawLine... why bother doing it
 *
 * This scene is about boxes falling into a crate, which was part of my old code
 * about creating point cloud of crates with boxes. Less than 20 lines of code
 * is necessary for visualization.
 *
 * Here a function calls function of CommonVisualizer is used. In the future
...
void add_ground(const btVector3 &scaling, const btVector3 &center,
                const btQuaternion &rotation, btDiscreteDynamicsWorld &world) {
  btScalar mass(0.0);
  ...
  world.addRigidBody(new btRigidBody(btRigidBody::btRigidBodyConstructionInfo(
      mass, new btDefaultMotionState(tf_ground), ground, inertia)));
}

void add_box(const btVector3 &scaling, const btVector3 &center,
             const btQuaternion &rotation, const btScalar &mass,
...
void run(const float interval, const int iteration, btDynamicsWorld &world) {
...
void set_crate(btDiscreteDynamicsWorld &world) {
  add_ground(btVector3(0.3, 0.2, 0.001) * scaling,
             btVector3(0.0, 0.0, -0.001) * scaling, btQuaternion(0, 0, 0),
             world);
  ...
  for (int i = 0; i < 8; ++i)
    for (int j = 0; j < 4; ++j) {
      ...
      add_ground(
          btVector3((0.6 - 0.08) / 16.0, (0.4 - 0.04) / 8.0, 0.0025) * scaling,
          btVector3(offset_x + step_x * i, offset_y + step_y * j, 0.0025) *
              scaling,
...
void add_boxes(btDiscreteDynamicsWorld &world, const int num = 6) {
  ...
  for (int i = 0; i < num; ++i) {
    btVector3 displace((double)(rand() % 1000 - 500) / 2000.0,
                       (double)(rand() % 1000 - 500) / 3000.0, 0.0);
