...
#include "include/DepthMapSaver.hpp"
...
// on my laptop clang++ test_screensaver.cpp
// -I/home/tlou/workspace/CommonVisualizer/ -lGL -lGLEW -lGLU -lglut -lglfw
// -lpng --std=c++11 -o test_screensaver

int main() {
  std::vector<float> positions;
  std::vector<float> colors;
  loco::CommonVisualizer vis(600, 800, "sphere", loco::color::BLACK);
  loco::DepthMapSaver saver;
  if (vis.isInited()) {
    ...
    std::generate(positions.begin(), positions.end(),
                  std::bind(gen_rand, 0.5f, 1000.0f));
    ...
    for (int count = 0; count < 20; ++count) {
      ...
      saver.saveImage(600, 800, vis.getDistance(),
                      std::string{"/tmp/"} + std::to_string(count) + ".png");
