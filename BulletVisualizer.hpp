...
namespace loco
{
  ...
  public:
    BulletVisualizer(const int height = 600, const int width = 800, const char *name = "default",
    ...
    void addDiscreteDynamicsWorld(const btDynamicsWorld &world, const std::vector<Vec> &colors)
    {
      ...
      {
        const btRigidBody *body = btRigidBody::upcast(world.getCollisionObjectArray()[ido]);
        ...
        if(strcmp(body->getCollisionShape()->getName(), "Box") == 0)
        {
          ...
          addBox(tf_zero, Vec{float(size.getX()), float(size.getY()), float(size.getZ())}, color);
        }
        else if(strcmp(body->getCollisionShape()->getName(), "Sphere") == 0)
        {
          float radius = float(((btSphereShape *) body->getCollisionShape())->getRadius());
          addPointCloud(std::vector<float>(3, 0.0f), color, radius);
        }
        else if(strcmp(body->getCollisionShape()->getName(), "CapsuleShape") == 0)
        {
          const btCapsuleShape *shape = (btCapsuleShape *) body->getCollisionShape();
          ...
          addCapsule(pos, std::vector<float>(1, radius), std::vector<Vec>(1, color));
        }
//        else if(strcmp(body->getCollisionShape()->getName(), "CylinderShape") == 0) // TODO is the name right?
//        {
//          const btCylinderShape *shape = (btCapsuleShape *) body->getCollisionShape();
//          float radius = float(shape->getRadius());
//          int id_up = shape->getUpAxis();
//          float half_height = float(shape->getHalfHeight());
//          std::vector<float> pos(6, 0.0f);
//          pos.at(id_up) = half_height;
//          pos.at(id_up + 3) = -half_height;
//          addCylinder(pos, std::vector<float>(1, radius), std::vector<Vec>(1, color));
//        }
        // TODO missing cone, irregular shapes and maybe softbody
      }
    }

    void addDiscreteDynamicsWorld(const btDynamicsWorld &world, const Vec &color = color::WHITE)
    {
      addDiscreteDynamicsWorld(world, std::vector<Vec>(world.getNumCollisionObjects(), color));
    }
    ...
    void updateTransformDiscreteDynamicsWorld(const btDynamicsWorld &world)
    {
      ...
      for(int ido = 0; ido < length; ++ido)
      {
        ...
        setTransform(ido, Transform{
            Vec{float(translation.getX()), float(translation.getY()), float(translation.getZ())},
            Vec{float(rotation.getX()), float(rotation.getY()), float(rotation.getZ()), float(rotation.getW())}});
      }
    }
  };
}
