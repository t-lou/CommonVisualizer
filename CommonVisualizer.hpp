...
namespace loco
{
   ...
   * @param id
   */
  void CommonVisualizer::dispErrorCompilation(const int length, const GLuint id, const GLenum &type)
  {
    std::string error;
   ...
   * @return
   */
  GLuint CommonVisualizer::createShader(const GLenum &shader_type, const char *shader_text)
  {
...
//      const int BUTTON_ZOOM = GLFW_MOUSE_BUTTON_RIGHT; // in case middle button doesn't work
  ...
  CommonVisualizer::CommonVisualizer(const int height, const int width, const char *name,
...
//      glutReshapeFunc(wrapper_resize_window);
   ...
   * clear all objects
   */
  void CommonVisualizer::resetScene()
  {
    _world.reset();
  }
   ...
   */
  bool CommonVisualizer::isInited() const
  {
   ...
   */
  void CommonVisualizer::addMesh(const std::vector<float> &vertices, const Vec &color)
  {
   ...
   * @param color
   */
  void CommonVisualizer::addMesh(const std::vector<float> &vertices, const std::vector<int> &index, const Vec &color)
  {
    if(_id_prog._id_program_unicolor_mesh == 0)
    {
      loadUnicolorMeshShader();
   ...
   */
  void CommonVisualizer::addMesh(const std::vector<float> &vertices, const std::vector<float> &colors)
  {
  ...
  void CommonVisualizer::addPointCloud(const std::vector<float> &points, const Vec &color,
                                       const float radius)
  {
    if(radius <= 0.0f)
    {
   ...
   * @param colors
   */
  void CommonVisualizer::addCone(const std::vector<float> &positions, const std::vector<float> &radius,
                                 const std::vector <Vec> &colors)
  {
   ...
   * @param color
   */
  void CommonVisualizer::addBox(const Transform &transform, const Vec &scale, const Vec &color)
  {
    if(_id_prog._id_program_unicolor_mesh == 0)
    {
      loadUnicolorMeshShader();
